...
import { Card, Empty, Input, Pill, Progress, Segmented, Select } from '@/components/ui'
...
type View = 'en_cours' | 'retard' | 'toutes'
...
export function Shipments() {
  ...
  const rows = useMemo(() => {
    ...
    return db.shipments
      .filter((s) => (view === 'en_cours' ? s.status === 'en_cours' : view === 'retard' ? shipmentLate(s) : true))
  ...
  const inTransit = db.shipments.filter((s) => s.stage === 'transit').length
  const arriving = db.shipments.filter((s) => s.status === 'en_cours' && daysUntil(s.eta) >= 0 && daysUntil(s.eta) <= 7).length
  const customs = db.shipments.filter((s) => s.stage === 'douane').length

  return (
    <>
      <PageHead title={t('ship.title')} subtitle={t('ship.subtitle')} />
      ...
      <Card flush>
        <div className="row wrap gap-3" style={{ padding: 'var(--sp-4) var(--sp-6)', borderBottom: '1px solid var(--hairline)' }}>
          ...
          />
          <Select value={mode} onChange={(e) => setMode(e.target.value as ShipmentMode | 'tous')} style={{ width: 'auto' }}>
          ...
          <Input placeholder={t('action.search')} value={query} onChange={(e) => setQuery(e.target.value)} style={{ maxWidth: 220 }} />
          <span className="grow" />
            ...
            <table className="table">
              <thead>
                <tr>
                  ...
                  <th>{t('ship.stage')}</th>
                  <th>{t('ship.eta')}</th>
                  <th className="num">{t('ship.freight')}</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((s) => (
                  <tr key={s.id} onClick={() => navigate(`/cargaisons/${s.id}`)}>
                    ...
                    <td className="t-small t-secondary">{t(`ship.m.${s.mode}` as 'ship.m.aerien')}</td>
                    <td>
                    ...
                    <td className="num t-small">{formatMoney(s.freightCost)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
